...
const Dashboard: React.FC<DashboardProps> = ({ children }) => {
	...
	return (
		<div className='mx-auto xs:mt-40 md:mt-24 slg:mt-16 max-w-screen-2xl sm:px-10 mb-32'>
							...
							<Link
								...
								className={`p-3 text-base grid w-fit slg:flex slg:w-full items-center gap-3 rounded-md border ${
									pathname.includes(item.href)
										? "text-[#54b22c] border-[#54b22c]"
										: "text-black"
								} hover:bg-gray-50 w-full hover:text-[#54b22c]`}
							>
								<item.icon
									className='flex-shrink-0 h-8 w-8 slg:h-5 slg:w-5 cursor-pointer'
									aria-hidden='true'
								/>
								<span className='inline-flex items-center justify-between text-xs slg:text-base font-medium w-full hover:text-[#54b22c]'>
									{item.title}
								</span>
							</Link>
						))}
						<span className='p-3 grid slg:flex items-center border gap-3 rounded-md hover:bg-gray-50 w-full hover:text-[#54b22c] text-base cursor-pointer'>
							<BiLogOut className='text-3xl slg:text-2xl' />
							<button
								onClick={handleLogOut}
								className='inline-flex items-center justify-between font-medium text-xs slg:text-base w-full hover:text-[#54b22c]'
							>
								Logout
							</button>
						</span>
